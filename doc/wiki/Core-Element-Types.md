...
- `class IjkGridRep(GridRep)` — `KINDS = ("IjkGrid",)`. The only kind that goes through the modal IjkGrid pipeline (I/J/K slicers + volume crop + threshold chain, range vs slice mode). Overrides:
  ...
  - `rendered_sources(self, ris)` — every proxy the per-view IjkGrid renders (slicers + volume + rep_data), with the deepest-visible-leaf proxy (`ijk._deepest_visible_leaf().pv_proxies.get(id(s))`) substituted; `None` → legacy.
  - `color_sources(self, ris)` — slicers + volume + threshold leaves (NOT rep_data, which is hidden upstream); `None` → legacy.
