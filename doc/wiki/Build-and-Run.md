...
1. **`builder`** (`FROM paraview:6.0.x`) — compiles ParaView (`build.bash`, `PVSB_GIT_TAG=v6.0.1`), then FESAPI + its deps, FETPAPI + its deps, and finally **FESPP** from `./fespp/` (copied in via `fespp-src-local*`). Output: `/work/pvsb-build/install` (ParaView) and `/work/ttl` (FESPP + libs).
