...
- `class Wellhead`
  ...
  - `delete(self)` — `pvsimple.Delete(self._source)` if a source was created.

**State.** None directly (operates on ParaView proxies, not Trame state).
...
- `class Tree`
  ...
  - `set_tree(self, data_assembly)` — rebuilds the three `_data_hierarchy_*` lists from the assembly root, dispatching each top-level node to reservoir/surface/well (via `dispatch_kind`, resolving Feature/Interpretation through `_resolve_dispatch_kind`), then sorts and writes `state.ui_subtree_reservoir/well/surface`.
