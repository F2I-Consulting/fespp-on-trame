...
- **`_render_panel_actions` buttons overflow up into the dockview tab row** (negative `top`, relying on an `overflow: visible` CSS rule forced on `.dv-content-container` in `shared/styles.py`). They `stopPropagation` on `mousedown`/`click` so dockview doesn't interpret the click as a tab-activation. The "Split right/below" buttons are commented out — the grid-split add-view path is WIP. Base `right` offset is 44px (92px when the toolbar is hidden), NOT 8: the far right of the tab row belongs to dockview's right-header-actions — the "+" add-view button — and at 8px this chrome sat underneath it, hiding the ⚙.
- **The "+" add-view button at the far right of the tab row is ptc's `rightHeaderActionsComponent`** (template name `ptc-multiview-add`, registered by `ptc.MultiView.__init__`). `FesppMultiView.__init__` re-registers that same `DivLayout` name after `super().__init__` to replace ptc's stock button (a `flat` VBtn whose surface background rendered as an anonymous white disc over the dark tab row) with a bar-chrome-styled one (text variant, white, tooltipped "Add a view"); clicking still calls `add_view()` directly. Note the `AddViewDialog` (kind + position picker) is wired but has **no trigger** — nothing sets `add_view_dialog_visible` to true in the current build.
