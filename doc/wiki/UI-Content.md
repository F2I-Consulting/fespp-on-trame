...
- **Single-view v1 trims (multi-view not shipped).** Beyond the "+" button below, v1 unmounts every multi-view affordance that read as unfinished UI on a lone view: the top **ToolsBand** is not rendered (`content.py` — in-view TC/MR toggles + the per-view ⚙ dialog + the tree's chart icon cover all of it), the camera-link **magnet** (`ViewLinkMenu`) is dropped from `_render_panel_camera_chrome`, the drawer's **"Target view:" strip** is unmounted (`drawer.py`), and the **ACTIVE pill + blue border** only show when 2+ render panels exist. All the underlying machinery stays alive for the V2 re-mount.
- **No user-clickable "add view" in V1.** `ptc.MultiView` installs an "add view" (+) button as dockview's `rightHeaderActionsComponent` (template name `ptc-multiview-add`) — a `flat` VBtn whose surface background rendered as an anonymous white disc over the dark tab row, covering the `_render_panel_actions` chrome anchored at `right:8px`. `FesppMultiView.__init__` re-registers that same `DivLayout` name **empty** after `super().__init__` (the last layout registered under a name wins): multi-view is not considered stable enough to expose in the v1 release. The analytics panels (stats / distribution / compare-stats) still create their tabs programmatically via `add_view(kind=...)`; ptc's watermark ("Create RenderView") remains as the only recovery path if the last panel is ever closed. The `AddViewDialog` (kind + position picker) stays wired but unreachable — nothing sets `add_view_dialog_visible` to true.
