...
- `class _FesppColorOpacityEditor(ptc.ColorOpacityEditor)`:
  ...
  - **Scalar range + log (continuous only).** `_resolve_active_lut()` → `(base, scoped-or-global LUT)` (same resolution as `update_scalar_range`); `_apply_scalar_range(lo, hi)` widens then `RescaleTransferFunction`s the target-view LUT and pushes; `apply_color_range` (Apply button / `keydown_enter`) reads `color_range_min`/`max` (ignores `max <= min`); `reset_color_range_to_data` recomputes the data range and rescales back; `_positive_floor(hi)` = a strictly-positive lower bound (four decades below max) for a log axis; `_sync_range_fields` mirrors a range into the Min/Max inputs (they never auto-apply, so this can't loop).
