...
- `_render_data_explorer_card` / `_render_data_explorer_title` / `_render_data_explorer_tabs` / `_render_data_explorer_body` — the title toolbar hosts a **Load** button (`v_if="load_mode === 'manual'"`, click → `controller.apply_pending_selection`) and a cog (click → `controller.drawer_options_open`). Tabs are a `VTabs` bound to `state.tab`; each tab label shows a live count, e.g. `Reservoir ({{ ui_subtree_reservoir.length }})`. The body mounts all three trees simultaneously, toggled with `v_show` (`self._tv.reservoir_tree()` etc.) so each tree keeps its expansion/selection/scroll state across tab switches. The `VTreeview`s run `items_registration="props"` (Vuetify >= 3.10, trame-vuetify pinned >= 3.2.2): collapsed children are NOT mounted — the default mounted every item at once, measured as the client half of the "no tree for 2-3 s" import stall and the bulk of the ~6000-DOM-node reflow behind slow tab switches. The upload overlay also shows "Reading EPC…" during the C++ parse (1.3-1.7 s of blocked event loop, previously a silent 100 % bar; the endpoint flushes + `await asyncio.sleep(0)` BEFORE the blocking loop or the message would ship after it).
...
- `_expand_selection_with_deps(curr_ids, prev_ids, tree) -> list` — the dependency cascade. **Additions:** checking a grouping kind adds all its *selectable* descendants; checking a `WellboreChannel`/`WellboreMarker` adds the sibling `WellboreTrajectory`; checking any node under a representation adds that representation (`tree.find_representation_node`). **Removals:** unchecking a representation or a grouping drops every descendant. Implicit additions are registered in the module-level `_cascade_added` set (per select var) so `_wire_select_to_active` can tell them apart from user clicks — a cascade-added node never grabs the activation. **Threshold guard:** a *unitary* uncheck of a property whose array feeds threshold entries (`threshold_dispatch.chain_entries_for_property`) is vetoed — the node is re-added and `ThresholdUnselectDialog` opens; its confirm deletes the entries in every view then re-applies the uncheck (which then passes). Bulk unselects bypass the guard.
- `_select_toolbar(tab, actions)` — the "Select / unselect ▾" dropdown above each tree tab. Each entry `(label, kinds)` offers BOTH bulk actions: a check-all button (`tree_select_kinds(tab, kinds, 'add')` — load) and an uncheck-all button (`'remove'` — unload; a grouping kind takes its whole subtree along, symmetric with unchecking the folder by hand). `close_on_content_click=False` keeps the menu open so select-then-unselect chains don't re-open it each time. The trajectory auto-add is scoped STRUCTURALLY to the frames: an element of a Frame / MarkerFrame (a log or a marker — their `kind` is the RESQML XML tag, i.e. a plain property kind for a log, never a "Wellbore…"-prefixed enum name; that wrong assumption made the auto-add a silent no-op since day one), or the frame folder itself, pulls in the well's "Trajectory" child. Completions/perforations trigger too — a perforation is COMPUTED from the trajectory (its MD values are converted to XYZ along it by the C++ mapper), so it renders nonsensically without the well path.
- `_wire_select_to_active(select_var, active_var, prev_var)` — registers a `@_state.change(select_var)` handler that sets `active_var` to the newly-added **user-checked** node: additions registered in `_cascade_added` (dependency expansion, geometry autoload on property check, forced BW trajectory) are filtered out, so checking a property deterministically activates the PROPERTY — not the geometry the cascade pulled in — and checking a BW doesn't steal the reservoir focus. When the active node leaves the selection, `_fallback_active` re-targets deterministically: the geometry of the same grid if still checked, else the node's rep if still checked, else nobody (`[]`) — never an arbitrary `curr[0]`, which could silently promote another property. Needed because Vuetify's `update_selected` always emits the *full* array, so binding active to `$event[0]` would pick the first-ever-selected node, not the last clicked. ⚠️ **A BULK select must pre-seed `prev_var`** — it adds a whole folder + all its leaves at once, so the surviving `new_ones` tail is an ARBITRARY node of the bulk (typically the grouping folder). See `tree_select_kinds`.
...
- `_select_checkbox_icon(select_var) -> str` / `_select_checkbox_color(select_var) -> str` — Vue ternary expressions producing the checkbox icon/color. Tri-state on groupings (`item.is_grouping` with `item.descendant_ids`): `mdi-checkbox-marked` when *all* descendants selected, `mdi-minus-box` when *some*, `mdi-checkbox-blank-outline` otherwise; binary on `item.id` for everything else. Click cycle (`tree_toggle_select`): empty → all; **partial OR all → empty** (an indeterminate box clears its subtree). Unchecking a grid geometry cascades to its sibling properties (same PropertiesFolder) in `_expand_selection_with_deps`; blocked wellbores keep their selection and drop to solid colour.
...
- `_eye_slot(controller)` — the per-view visibility/coloring chips, gated on the server-computed `item.eye` token (`'rep'` / `'array'` / `'marker'`, absent on Frame/MarkerFrame folders and groupings). Three independent blocks:
  Unified styling (2026-08-19 UX batch): every chip pairs a BLACK view
  label (rendered only when >1 render panels; bold when that panel
  shows/activates the item) with one eye pair for all node kinds —
  filled blue `mdi-eye` when visible/active, and the inline
  `_SLASHED_EYE_SVG` (blue OUTLINE eye + black slash, raw SVG because
  mdi tints the slash) when hidden/inactive. A grid geometry loading
  while one of its blocked wellbores is present is born hidden in every
  view (see data_load), and a NEW blocked wellbore closes its grid
  geometry's eye.
  - **Rep eye** (`item.eye === 'rep'`, path in `ui_loaded_rep_paths`): visibility per panel via `ui_hidden_rep_paths_by_view`. Click → `controller.toggle_rep_visibility([item.path, panel.id])`. Visibility ⊥ colouring: hiding never touches the active array.
  - **Array eye** (`item.eye === 'array'`, path in `ui_loaded_array_paths`): active when `ui_active_array_by_rep_by_view[panel.id][item.rep_path] === item.path`. Click → `controller.toggle_dataarray_color([item.path, panel.id])`.
  - **Marker eye** (`item.eye === 'marker'`, path in `ui_loaded_marker_paths`): multi-select visibility via `ui_visible_marker_paths_by_view`. Click → `controller.toggle_marker_visibility([item.path, panel.id])`.
  - **Stats toggle** (`_stats_slot`): pinned = filled teal `mdi-chart-box`; unpinned = the same teal glyph slashed black (`_SLASHED_CHART_SVG`) — never grey (read as disabled). The UnstructuredGrid kind icon is the `fespp-tetra` sentinel rendered as the inline `_TETRA_SVG` wireframe tetrahedron.
...
- `_render_body(self)` — header with a root "Add union" button (`mdi-set-all`, disabled until `ui_active_node_reservoir_array_name` is set — the RESERVOIR-scoped twin of `active_color_array_name`, immune to well/surface tab activations clearing the shared var) that emits `ui_threshold_pending_action = { action: 'add', parent: null }`. Then a `v_for` over `ui_threshold_chain`: each entry row (indented by `entry._depth * 12px`) has a visibility eye (`set_visible`), the array name + `[low .. high]`, an "Add intersection" button (`{ action: 'add', parent: entry.name }`), and a delete button. Below each row, one of three `VRangeSlider`s by `entry.kind`: **Discrete** (integer `step=1`), **Categorical** (`step=1` with labeled `ticks` from `entry.labels`, `show-ticks="always"`), or **Continuous** (fine `step = range/1000`). All emit `{ action: 'set_range', name, low, high }` on release.

The add buttons are additionally disabled while `ui_active_node_reservoir_pending` is true ("selected but not loaded yet" — manual load mode or mid-load), with a "*(load the selection first)*" hint. When a **BlockedWellbore** is the active reservoir node, the activator remaps the reservoir rep to its supporting grid, so the panel stays visible **read-only** with the grid's chain (no active property → buttons greyed) instead of vanishing.

**State.** Reads `ui_threshold_chain`, `ui_active_node_reservoir_array_name`, `ui_active_node_reservoir_pending`. Writes the sentinel `ui_threshold_pending_action`.
...
**Responsibility.** The continuous-property LUT/PWF color-opacity editor (subclass of ptc's editor) with an added hexa NaN-color picker, a scalar-range (min/max) rescale + logarithmic-scale toggle, and the per-view scoped-LUT plumbing for it.
...
- `class _FesppColorOpacityEditor(ptc.ColorOpacityEditor)`:
  ...
  - **Scalar range + log (continuous only).** `_resolve_active_lut()` → `(base, scoped-or-global LUT)` (same resolution as `update_scalar_range`); `_apply_scalar_range(lo, hi, pin=True)` widens then `RescaleTransferFunction`s the target-view LUT **and its scoped PWF in lockstep** (`_resolve_target_pwf` via `scene.get_or_create_pwf`) and pushes; `apply_color_range` (Apply button / `keydown_enter`) reads `color_range_min`/`max` (ignores `max <= min`); `reset_color_range_to_data` UNPINS, recomputes the data range and rescales back (`pin=False`); `_positive_floor(hi)` = a strictly-positive lower bound (four decades below max) for a log axis; `_sync_range_fields` mirrors a range into the Min/Max inputs (they never auto-apply, so this can't loop).
  - **Instance-bound buttons — the OTHER reason the feature looked dead.** Apply/Reset bind `self.apply_color_range` / `self.reset_color_range_to_data` directly. The first iteration registered them as `controller.fespp_apply_color_range`, and the diff dialog's `_DiffColorOpacityEditor` (same `__init__`, constructed later) overwrote that name with ITS bound method — whose inverted guard only passes while the diff dialog is open, so every Apply click silently no-oped.
  - **Below/above range colours.** One row in colour-bar reading order: below swatch, Min, Max, above swatch (`_range_color_swatch`). Empty state (`below/above_range_color`) = VTK clamp default; the swatch then shows the preset's actual first/last stop colour (`*_range_auto_color`, mirrored from `lut.RGBPoints` in `on_scalar_range_changed`). Picking a colour sets `Use{Below,Above}RangeColor` + `{Below,Above}RangeColor` on the scoped LUT (`_apply_range_color`, no-op when unchanged so the programmatic reflect can't re-render). HEX mode only — no alpha slider: VTK has no below/above-range opacity, and the picker seeds BLACK on an empty v_model (dragging alpha first emitted #000000XX → every out-of-range cell painted black); one-way `model_value` with the auto colour as fallback avoids that trap. PV 6.0.1's scalar bar exposes no below/above swatch properties — what the bar draws is whatever VTK does natively.
  - **`_apply_scalar_range` ends by rebuilding the COE widget state** (`controller.update_color_editor`): the graph's colour stops carry SCALAR positions, so leaving them at the old range after a rescale makes the canvas normalise out of [0,1] and the gradient breaks.
  - **NaN colour targets the SCOPED LUT** (`_resolve_active_lut`), not the global singleton — displays render with the per-(view, array) LUT, so the old global write silently did nothing under the per-view model. **Preset and opacity-mapping too**: `on_preset_name_changed` fully replaces ptc's parent (which applied the preset to the global LUT + rescaled via the active source's rep — a total no-op per-view); it applies to the scoped LUT, re-applies the pre-preset scalar range (pinned or data), preserves the NaN alpha byte, rebuilds the COE via `update_color_editor`. `on_opacities_changed` flips `EnableOpacityMapping` on the scoped LUT and never forces it back off while a below/above range alpha needs it.
  - **Cosmetics per user feedback**: panel chip shows the PRESET name ("Fast", …) or "Categories" instead of "LUT/PWF"; Min/Max inputs are plain text (no number-spinner arrows) mirrored at 3 decimals; the scalar bar drops the "Component" suffix (`ComponentTitle=""` — scalars only in RESQML properties today); the AppBar title is pinned to `min-width: max-content` (flex ellipsis truncated it at exactly 100% zoom).
  - **The range PIN — why the feature works at all.** Applying a custom range sets `AutomaticRescaleRangeMode='Never'` on the LUT proxy itself; `leaf_rep.range_is_pinned(lut)` reads it back, and both auto-rescale sites — `apply_color_array`'s client-side force-rescale and `leaf_rep.rescale_to_range` — stand down on a pinned LUT. Without the pin, the very next eye click / activation / time step snapped the range back to the data and the feature looked dead (the first iteration's actual bug). The pin travels with the scoped LUT (per view × array): no side bookkeeping, and the wellbore mirror (shared LUT) follows the pinned range for free. `on_scalar_range_changed` mirrors the PINNED range (read off `lut.RGBPoints`) into the Min/Max inputs instead of the recomputed data range while pinned.
  ...
  - `@change("scalar_range") on_scalar_range_changed` — mirrors the recomputed range into the Min/Max inputs and reflects the LUT's `UseLogScale` in the switch. `@change("color_use_log") on_color_use_log_changed` — toggles `lut.UseLogScale` (+ `MapControlPointsToLog/LinearSpace`), **no-ops when the LUT already matches the request** so the programmatic reflect above can't re-remap the control points; clamps the lower bound positive (via `_positive_floor`) before enabling.

**State.** Reads/writes `nan_color`, `scalar_range`, `color_range_min`, `color_range_max`, `color_use_log`, `active_color_array_name`, `active_color_array_path`, `active_representation_path`, `drawer_target_view_id`, `fespp_active_panel_id`, `diff_colors_dialog_visible`, `colors`, `opacities`, `preset_name`.

**Collaborators.** `ptc.ColorOpacityEditor`, `source_resolver` (`resolve_target_scoped_lut`, `channel_source_for`, `nondegenerate_range`, `render_and_push_target`), `scene_registry` from server context, `pvsimple`. Registers `controller.fespp_apply_color_range` / `fespp_reset_color_range` (Apply/Reset buttons). Instantiated by `SolidColorPanel` for continuous properties.

**Gotchas.** The override of `update_scalar_range` exists because ptc's `self.source_proxy` is `GetActiveSource()` = the legacy shared `ExtractBlock`, which lacks MR `_real_<idx>` arrays; the per-view IjkGrid extractor must be force-updated with a full pipeline pass or `GetDataInformation()` reports zero cells. ptc's writers mutate proxies without calling `Render`, hence the explicit `render_and_push_target` in every handler. **Log scale needs a strictly-positive lower bound and ParaView does NOT auto-clamp it** — `UseLogScale=1` on a range touching 0/negatives leaves the RGBPoints untouched (broken mapping), so `_positive_floor` rescales first. The `color_use_log` handler is made idempotent (skip when `UseLogScale` already matches) because `on_scalar_range_changed` writes `color_use_log` from the LUT on every range refresh, and `MapControlPointsToLogSpace` is NOT idempotent (each call re-warps the stops).
...
- `class CategoricalColorEditor(html.Div)`:
  ...
  - `_refresh(self, array_name)` — scans the active source's VTK array for unique integer values (dropping RESQML integer NullValue sentinels via `extract_block.is_null_sentinel` — INT32_MAX inactive cells would otherwise show up as an unnamed extra category AND get written back into the LUT `Annotations`), reads the per-view scoped LUT's `Annotations`/`IndexedColors`/`IndexedOpacities`, seeds a categorical preset on first activation (tries `"Categorical 1"`, `"Set 1"`, …), builds `categorical_entries`, then pushes `IndexedLookup=1` + `EnableOpacityMapping=1` + colors/opacities/annotations back. Deliberately does **not** call `Render` (the activation handler renders).
...
### `fespp_on_trame/app/ui/shared/widget/threshold_unselect_dialog.py`
**Responsibility.** Confirm dialog for unchecking a property that feeds threshold entries. Opened by the selection guard in `tree_views._expand_selection_with_deps` (which vetoes the uncheck first).

**Key classes / functions.**
- `class ThresholdUnselectDialog` — `render()` draws a `VDialog` bound to `thr_unselect_dialog_visible`; body interpolates the property title + entry count from `thr_unselect_dialog`. "Cancel" just closes (the veto already restored the selection); "Delete & unselect" calls `controller.confirm_unselect_property`, which deletes the matching entries in every view then re-applies the uncheck.

**State.** Reads `thr_unselect_dialog_visible`, `thr_unselect_dialog`.
