...
- `_eye_slot(controller)` — the per-view visibility/coloring chips, gated on the server-computed `item.eye` token (`'rep'` / `'array'` / `'marker'`, absent on Frame/MarkerFrame folders and groupings). Three independent blocks:
  - **Rep eye** (`item.eye === 'rep'`, path in `ui_loaded_rep_paths`): one `mdi-eye`/`mdi-eye-closed` per render panel — **blue when shown, grey when hidden** (`ui_hidden_rep_paths_by_view`), *always*, whether or not the panel colours by an array. Click → `controller.toggle_rep_visibility([item.path, panel.id])`. It used to dim under an active array, back when its first click meant "give up the colouring"; now that it is the ONLY visibility control, dimming misread as disabled exactly when it matters most (hiding a coloured rep). Blue = visible, purple = colouring: two colours, two orthogonal roles.
