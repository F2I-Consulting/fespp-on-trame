...
- `class _FesppColorOpacityEditor(ptc.ColorOpacityEditor)`:
  ...
  - **NaN colour targets the SCOPED LUT** (`_resolve_active_lut`), not the global singleton — displays render with the per-(view, array) LUT, so the old global write silently did nothing under the per-view model.
