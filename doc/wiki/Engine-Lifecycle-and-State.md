...
**Responsibility.** Handle tree eye-icon clicks on a representation node and on a wellbore-marker leaf. The rep eye is a 3-state affordance (hidden / visible+SolidColor / visible+array); the marker eye is a plain multi-select show/hide.

**Key classes / functions.**
- `toggle_rep_visibility(state, controller, server, source_registry, rep_path, panel_id=None, tree=None)` — the rep eye handler. Resolves `(view, html_view)` and `bucket_key` (panel_id, else active panel, else `"_active"`). If the rep currently has an active array in this panel AND isn't hidden AND isn't a wellbore `Frame`, the click means "clear coloring, keep geometry" → delegates to `_clear_active_array`. Otherwise it flips the rep in `ui_hidden_rep_paths_by_view[bucket_key]`, mirrors to the legacy global `ui_hidden_rep_paths` iff active panel, resolves sources via `source_resolver.sources_for_rep_path`, and Shows/Hides them. On show: for an IjkGrid it calls `ijk.show(view=view)` (the per-mode logic is intricate); for an ExtractBlock rep it re-asserts `Representation` (`state.representation_active`) and `_apply_default_tint(d, grid_color)` on each source's display because non-original panels start with PV defaults. Finally renders + pushes the html_view (or `controller.view_update()`).
...
- `_clear_active_array(state, controller, server, source_registry, tree, rep_path, panel_id, view, html_view)` — drops the active-array binding in `ui_active_array_by_rep_by_view[bucket_key]` (mirrors the global map iff active panel), pushes SolidColor via `source_resolver.apply_color_array(..., None)`, sweeps stale bars via `hide_unused_scalar_bars`, renders.

**State.** Reads/writes `ui_active_array_by_rep_by_view`, `ui_active_array_by_rep`, `ui_hidden_rep_paths_by_view`, `ui_hidden_rep_paths`, `ui_visible_marker_paths_by_view`. Reads `representation_active`, `solid_color_by_rep`.
...
**Gotchas.**
- **Wellbore `Frame` reps skip the 3-state intermediate** (`is_frame` short-circuits the clear-coloring branch): a frame has no own geometry, so the "clear coloring, keep geometry" state would leave the log painted SolidColor instead of hidden — so the frame eye is a plain show/hide.
...
- `render_and_push_target(controller)` — render the drawer target's view + push its frame via `controller.view_update_for(panel_id)` (falls back to `view_update`).
