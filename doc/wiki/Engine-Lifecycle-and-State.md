...
- `layout_visible_scalar_bars(view=None)` — hands every visible colour bar to ParaView's own placement algorithm (`vtkSMScalarBarWidgetRepresentationProxy.PlaceInView`, the code the desktop GUI runs): each bar takes the first spot not occupied by the other visible bars. Stable iteration order makes re-running idempotent. Called at the end of `apply_color_array` (after the sweep) and of `reassert_active_scalar_bars`.
