...
> **⚠️ Refactor update (`refactoring` branch).** That target-view precedence, the `(panel, active rep)` lookup, the panel pv_view resolution, and the render + dual-push were previously copy-pasted across `slicer` / `slice` / `clip` / `threshold` dispatch. They now live in **`engine/view_routing.py`** (`target_panel_id(state, scene_registry=None)` — 3-tier with a registry, 2-tier without; `resolve_rep`; `scene_pv_view`; `render_and_push` / `push_to_clients`; and `resolve_active_render_panel(state, raw_id=None)` — coerces a panel id to a RENDER panel: with a stats/distribution tab focused, the raw active id points at a non-render panel and colour/visibility buckets written under it are silently lost; `panel_resolver.active_panel_id` and every load-time bucket write in `data_load` route through this coercion). The four dispatchers delegate, keeping only their own pv_view nuance — so the per-file descriptions below that re-explain this resolution reflect the pre-refactor copies. The stats/distribution/compare cluster (`stats_dispatch`, `distribution_dispatch`, `compare_matrix`) is property-driven ("Brique B") rather than view-driven, and builds transient PV filter chains that must snapshot/restore the PV active source + active view so the main render path never sees a transient proxy.
...
- `chain_entries_for_property(state, scene_registry, source_registry, tree, prop_node_id)` — `[(view_id, entry_name), ...]` of the threshold entries fed by a property node, across every view's chain of its grid (matches the sanitized base array name and its MR `_real_<idx>` variants; `view_id=None` for the legacy single-registry provider). Powers the unselect guard: the tree veto asks it whether the uncheck needs a confirm, and `controller.confirm_unselect_property` uses it again to delete the entries before re-applying the uncheck.
- `threshold_add(state, controller, scene_registry, source_registry, activator, view, parent_name=None, array=None, view_id=None, tree=None)` — defaults `array` to `state.ui_active_node_reservoir_array_name` (the reservoir-scoped active property, immune to well/surface activations) falling back to `state.active_color_array_name`, runs MR resolution when `tree` is passed, calls `provider.add_threshold(...)`, republishes, refreshes the activator, hides unused scalar bars, renders.
...
**State.** Writes `ui_threshold_chain`, `ui_threshold_arrays_available`, `ui_threshold_pending_action` (cleared). Reads `ui_active_node_reservoir_type_rep`, `ui_active_node_reservoir_rep_path`, `ui_active_node_reservoir_array_name`, `active_representation_path`, `active_color_array_name`, `fespp_data_selectors`, `ui_active_array_by_rep_by_view`, `drawer_target_view_id`, `fespp_active_panel_id`. Also reads/writes `ui_active_realization_by_array_by_view` indirectly via `realization_dispatch`.
