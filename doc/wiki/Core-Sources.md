...
- `show(view=None)` — **The visibility brain.** Slice/clip active → hide all sources. Else re-assert Representation+tint+Scale on every source (needed for views created later than `set_node_id`), then: slice mode shows the per-axis crops (rep_data as fallback when no slicer visible); range mode shows `slicervolume` on a subset or rep_data at full extent (see `_is_range_full_extent`). When a chain is visible, each would-be-visible source is replaced by its deepest-visible-leaf Threshold proxy via `_show_source_or_chain`.
...
- `_primary_range_source()`, `_active_upstreams()`, `_deepest_visible_leaf()`, `_ancestors_all_visible()`, `_show_source_or_chain()`, `_hide_chain_for()`.
